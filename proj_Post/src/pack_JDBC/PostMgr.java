...
public class PostMgr {
	...
	// 게시판 리스트
	public Vector<PostBean> getPostList(String keyField, String keyWord,
			int start, int end) {
		Connection con = null;
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select * from tblPost order by ref desc, pos limit ?, ?";
				...
				sql = "select * from  tblPost where " + keyField + " like ? ";
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			con = pool.getConnection();
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(*) from tblPost";
				pstmt = con.prepareStatement(sql);
			} else {
				sql = "select count(*) from  tblPost where " + keyField + " like ? ";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%" + keyWord + "%");
			}
			rs = pstmt.executeQuery();
	...
	public void insertPost(HttpServletRequest req) {
		...
		try {
			con = pool.getConnection();
			sql = "select max(num)  from tblPost";
			pstmt = con.prepareStatement(sql);
			rs = pstmt.executeQuery();
			int ref = 1;
			if (rs.next())
	...
	// 게시물 리턴
	public PostBean getPost(int num) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			con = pool.getConnection();
			sql = "select * from tblPost where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			if (rs.next()) {
	...
	public void upCount(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblPost set count=count+1 where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void deletePost(int num) {
		...
		try {
			con = pool.getConnection();
			sql = "select filename from tblPost where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			...
			sql = "delete from tblPost where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
	...
	public void updatePost(PostBean bean) {
		...
		try {
			con = pool.getConnection();
			sql = "update tblPost set name=?,subject=?,content=? where num=?";
			pstmt = con.prepareStatement(sql);
	...
	// 게시물 답변
	public void replyPost(PostBean bean) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		try {
			con = pool.getConnection();
			sql = "insert tblPost (name,content,subject,ref,pos,depth,regdate,pass,count,ip)";
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
	...
	// 답변에 위치값 증가
	public void replyUpPost(int ref, int pos) {
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		try {
			con = pool.getConnection();
			sql = "update tblPost set pos = pos + 1 where ref=? and pos > ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, ref);
			pstmt.setInt(2, pos);
			pstmt.executeUpdate();

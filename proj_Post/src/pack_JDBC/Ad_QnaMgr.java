...
public class Ad_QnaMgr {
	...
	public Vector<Ad_QnABean> getQnAList(String keyField, String keyWord, int start, int end) {
		...
		try {
			...
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select * from tblQnA where del= 0 and answer<3 order by ref desc, pos limit ?, ?";
				...
				sql = "select * from  tblQnA where " + keyField + " like ? and del=0 and answer<3 ";
	...
	public int getTotalCount(String keyField, String keyWord) {
		...
		try {
			con = pool.getConnection();
			if (keyWord.equals("null") || keyWord.equals("")) {
				sql = "select count(*) from tblQnA where del=0 and answer<3 ";
				pstmt = con.prepareStatement(sql);
			} else {

				sql = "select count(*) from  tblQnA where " + keyField + " like ? and del=0 and answer<3";
				pstmt = con.prepareStatement(sql);
				pstmt.setString(1, "%" + keyWord + "%");
	...
	public void insertQnA(HttpServletRequest req) {
		...
		try {
			...
			sql = "insert into tblQnA(id, content, subject, ref, pos, depth, regdate, pass, ip, answer, del) ";
